...
planeContext:
  inputs:
    - what: Journey map for the same scope
      description: >-
        Work item under `Knowledge Base/{{track}}/Journeys/*`. Ask the user
        which journey if there are multiple.
    ...
    - what: 'System context — systems, tools, and back-stage teams involved'
      description: >-
        Work items under *Systems*, *Architecture*, *Operations*, *Teams* (or
        with 'system', 'architecture', 'team' in titles). If little is
        available, the back-stage cells will be flagged as research gaps.
...
- **Inputs** — CLARA will search the programme's Plane project for journey map for the same scope; persona for the journey; system context — systems, tools, and back-stage teams involved. You can also paste fresh material if it's not in Plane yet.

## Where the output lands

`Knowledge Base/{{track}}/Service blueprints/{{journey-scope}}` inside your programme's Plane project.
